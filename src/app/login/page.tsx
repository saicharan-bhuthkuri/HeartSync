      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <Card variant="glass" className="py-8 px-4 sm:px-10 border-rose-500/10 bg-white/80 backdrop-blur-md shadow-lg shadow-rose-500/5 rounded-[32px] relative overflow-hidden">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            {error && (
              <div className="p-3 rounded-2xl bg-red-500/10 border border-red-500/20 text-xs font-semibold text-red-600">
                {error}
              </div>
            )}
            ...
            <div>
              <Button
                type="submit"
                className="w-full py-3 rounded-2xl font-extrabold tracking-wide uppercase shadow-md shadow-pink-500/10 border-0 bg-linear-to-r from-pink-500 to-rose-450 hover:from-pink-600 hover:to-rose-550 text-white"
                isLoading={isLoading}
              >
                Sign In
              </Button>
...
export default function LoginPage() {
  return (
    <Suspense fallback={
      <div className="flex-1 flex flex-col justify-center py-12 px-4 text-center bg-linear-to-b from-[#fefaf8] to-[#fdf4f6] min-h-screen text-[#3a1e22]">
